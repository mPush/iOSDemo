...
- (void)getAuthCode{
    ...
    [PushManager authSms:@"YOUR MOBILE" timeInterval:3 finished:^(NSString *code, NSString *errorMsg) {
        NSLog(@"即将发送的验证码 = %@ （此处返回仅为了利于判断用户输入是否正确！）, errorMsg = %@",code,errorMsg);
        _contentLabel.text = [NSString stringWithFormat:@"验证码 = %@ \n errorMsg = %@",code,errorMsg];
    }];
}
